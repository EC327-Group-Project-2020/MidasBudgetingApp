...
@Database(entities = {User.class}, version = 3) //change the version with any changes to the schema of the database
    ...
    static AppDatabase getDatabase(final Context context) {
        if (dbINSTANCE == null) {
            synchronized (AppDatabase.class) {
                if (dbINSTANCE == null) {
                    dbINSTANCE = Room.databaseBuilder(context.getApplicationContext(), AppDatabase.class, "user_database").addCallback(sRoomDatabaseCallback).build();
                }
            }
        }
        return dbINSTANCE;
